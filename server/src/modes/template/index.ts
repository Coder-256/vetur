...
export function getVueHTMLMode(
  ...
  const embeddedDocuments = getLanguageModelCache<TextDocument>(
    10,
    60,
    document => documentRegions.get(document).getEmbeddedDocument("vue-html")
  );
  const vueDocuments = getLanguageModelCache<HTMLDocument>(10, 60, document =>
  ...
  return {
    ...
    doComplete(document: TextDocument, position: Position) {
      ...
      const tagProviders = enabledTagProviders.concat(
        getComponentTags(components)
      );
      return doComplete(
        embedded,
        position,
        vueDocuments.get(embedded),
        tagProviders,
        config.emmet
      );
    },
    ...
    format(
      ...
      return htmlFormat(document, range, config.vetur
