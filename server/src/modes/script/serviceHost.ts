        ...
        const doc =
          ...
          jsDocuments.get(
            TextDocument.create(
              ...
              "vue",
              0,
              ts.sys.readFile(resolvedFileName) || ""
            )
          );
        const extension =
...
function defaultIgnorePatterns(workspacePath: string) {
  ...
  const gitignore = ts.findConfigFile(
    workspacePath,
    ts.sys.fileExists,
    ".gitignore"
  );
...
function inferIsOldVersion(workspacePath: string): boolean {
  const packageJSONPath = ts.findConfigFile(
    workspacePath,
    ts.sys.fileExists,
    "package.json"
  );
  try {
    const packageJSON =
      packageJSONPath && JSON.parse(ts.sys.readFile(packageJSONPath)!);
    const vueStr =
